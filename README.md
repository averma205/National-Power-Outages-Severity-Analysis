...
#### **Evaluation Metric**
- **Metric**: Prediction accuracy (`CORRECT.PERCENT`) at the state level.
  - **Reason**: Accuracy is an appropriate metric to evaluate the performance of a multiclass classification model for state-level predictions. It quantifies the percentage of correct predictions made by the model for each group.
...
3. **Visualization**:
   - A scatter plot was created to display the relationship between the number of outages (`NUM.OUTAGES`) and prediction accuracy (`CORRECT.PERCENT`) for each state.
